...
public class AutoRed extends CommandOpMode {
    ...
    private final Pose startPose = new Pose(136, 33, Math.toRadians(90));  // Starting position
    private final Pose scorePose = new Pose(126, 18, Math.toRadians(135)); // Scoring position

    private final Pose pickup1Pose = new Pose(119.5, 19, Math.toRadians(157)); // First sample pickup
    private final Pose pickup2Pose = new Pose(119.5, 19, Math.toRadians(187)); // Second sample pickup
            ...
            case 2:
                if (!follower.isBusy()) { // Ensure it only moves when properly in this state
                    ...
                    if (pathTimer.getElapsedTimeSeconds() > 4.75){
                        CommandScheduler.getInstance().schedule(new InstantCommand(clawSubsystem::specimenPosition));
                        CommandScheduler.getInstance().schedule(new InstantCommand(clawSubsystem::drop));
                ...
                if (!follower.isBusy()){
                    CommandScheduler.getInstance().schedule(
                            new SequentialCommandGroup(
                                    new InstantCommand(armSubsystem::collectPosition, armSubsystem),
                                    new WaitCommand(2000),
                                    new InstantCommand(clawSubsystem::drop, clawSubsystem)
                            )
                    );
                    setPathState(4);
                }
                ...
                if (!follower.isBusy()){
                    ...
                    CommandScheduler.getInstance().schedule(
                            new SequentialCommandGroup(
                                    ...
                                    new InstantCommand(armSubsystem::basketPosition, armSubsystem),
                                    new InstantCommand(clawSubsystem::basketPosition, clawSubsystem)
                            )
                    );
                    setPathState(5);
                }
...
*/
    ...
    public void initialize() {
        ...
        CommandScheduler.getInstance().schedule(new AutonomousPathCommandRed(this));
